...
b_heat  %>% 
  ...
  ggplot(., aes(x=Species, y=station_id, fill=type)) + 
...
plotting_drop_outs_all %>% 
  ...
  ) +scale_x_log10(limits=c(10^0, 10^3), breaks=10^(0:3),labels=c(expression(10^0),expression(10^1),expression(10^2),expression(10^3)))-> sup_fig_1
...
plotting_drop_outs_all %>%
  ...
  filter(., ID_mifish !="Paralepididae") %>% 
  mutate(count_non_detections = if_else(larval_counts >0, 0,1)) %>% 
  dplyr::select(count_non_detections,mean_prop_reads) %>% 
  filter(., count_non_detections >0) %>% 
  arrange(desc(mean_prop_reads))


ggsave(
  sup_fig_2,
  file = here::here("analysis", "figures", "Figure_S3_non_detections.png"),
  width = 12,
  height = 8
)
