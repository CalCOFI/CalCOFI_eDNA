...
drop.these.mifish <- dat.mifish %>% group_by(ID_mifish) %>% tally(nReads > 0) %>% filter(n<11) %>% pull(ID_mifish) #Note change here to filter out rarely occurring species
dat.mifish <- dat.mifish %>%  filter (!ID_mifish %in% drop.these.mifish)
...
keepstations <- dat.count$station_id %>% intersect(dat.mifish$station_id) 
...
species_mapping <- read.csv(here("data","20210622_species_mapping_file.csv")) %>%
...
species_mapping <- species_mapping %>% filter(species_mapping$Unique_ID != "Clupeiformes")
species_mapping$Unique_ID[grepl("Sebastes", species_mapping$Unique_ID)] <- "Sebastes"   #synonymizing Sebastes in counts
species_mapping <- distinct(species_mapping)
names(species_mapping) <- c("ID_main", "ID_mifish", "ID_count")
species_mapping <- species_mapping %>% arrange(ID_main,ID_count,ID_mifish)
...
pos.all <- pos.count %>% dplyr::select(ID_main,station_id) %>%
  ...
  full_join(.,species_mapping %>% dplyr::select(ID_main,ID_count,ID_mifish)) 
  ...
  species_mapping$sp_index_main <- match(species_mapping$ID_main, unique(species_mapping$ID_main))
  ...
  I_main <- length(species_mapping$sp_index_main)
    ...
    SP_count <- species_mapping$ID_count[!is.na(species_mapping$sp_index_count)] %>% unique()
    I_count <- length(SP_count)

    SP_mifish <- species_mapping$ID_mifish[!is.na(species_mapping$sp_index_mifish)] %>% unique()
    ...
    ## make a matrix from the main list to each primer / data
      ...
      for(i in 1:I_count){
        M_to_count[species_mapping$sp_index_count[SP_count[i] == species_mapping$ID_count],
                   species_mapping$sp_index_main[SP_count[i] == species_mapping$ID_count]] <- 1
      }
      for(i in 1:I_mifish){
        M_to_mifish[species_mapping$sp_index_mifish[SP_mifish[i] == species_mapping$ID_mifish],
                    species_mapping$sp_index_main[SP_mifish[i] == species_mapping$ID_mifish]] <- 1
      }

      colnames(M_to_count)  <- species_mapping$ID_main
      colnames(M_to_mifish) <- species_mapping$ID_main
      rownames(M_to_count)   <- species_mapping %>% filter(!is.na(species_mapping$sp_index_count)) %>% dplyr::select(ID_count,sp_index_count) %>%
                                  distinct() %>% pull(ID_count)
      rownames(M_to_mifish)  <- species_mapping %>% filter(!is.na(species_mapping$sp_index_mifish)) %>% dplyr::select(ID_mifish,sp_index_mifish) %>%
  ...
  #we want to keep all stations/species for which there is at least one observation in one dataset (count or mifish)
    Obs <- full_join(pos.all %>% dplyr::select(ID_main,station_id),
...
###############################
  ...
  stanMod = stan(file = here("analysis","StanModel","Mifish_Joint_Model.stan") ,data = stan_data,
  ...
  TRACE <- list()
  TRACE[[as.name("Var")]] <- traceplot(stanMod,pars=c("lp__", "tau_mifish_0","tau_mifish_1",
                                                      "log_eta_mifish[1]","log_eta_mifish[11]","log_eta_mifish[31]",
                                                      "log_eta_mifish[70]","log_eta_mifish[83]","log_eta_mifish[99]"
                                                      ),inc_warmup=FALSE)
  ...
  mifish_pred_tau <- exp(pars$tau_mifish_0 + apply(pars$log_lambda_mifish,2,"*",pars$tau_mifish_1))
  
  D_mifish <- D_mifish %>% mutate(post_mean = colMeans(mifish_pred), 
                                  post_median = apply(mifish_pred,2,median),
                                  post_tau = exp(mean(pars$tau_mifish_0) + mean(pars$tau_mifish_1) * log(post_median)))                               
  ...
  # For just Anchovy
  po_2 <- ggplot(D_mifish %>% filter(ID_mifish =="Engraulis mordax")) +
    geom_point(aes(y=nReads,x=post_median, col=station_id),alpha=0.5) +
    scale_x_continuous(trans="log") +
    scale_y_continuous(trans="log") +
    geom_abline(intercept=0,slope=1,col="red") +
    theme_bw()
  ...
  # For just Sardine
  po_3 <- ggplot(D_mifish %>% filter(ID_mifish =="Sardinops sagax")) +
    geom_point(aes(y=nReads,x=post_median, col=station_id),alpha=0.5) +
    scale_x_continuous(trans="log") +
    scale_y_continuous(trans="log") +
    geom_abline(intercept=0,slope=1,col="red") +
  ...
  # General plot of how sd of Neg binom changes with sample size.
    ...
    tau <- exp(mean(pars$tau_mifish_0) + mean(pars$tau_mifish_1) * NB.log.mean)
  ...
  # Pull out the # smallest predicted mean, 
    ...
    temp.q.10 <- D_mifish %>% filter(post_median > quantile(D_mifish$post_median,0.1)) %>% 
                    slice(which.min(post_median))
    temp.q.25 <- D_mifish %>% filter(post_median > quantile(D_mifish$post_median,0.25)) %>% 
                  slice(which.min(post_median))
    temp.q.50 <- D_mifish %>% filter(post_median > quantile(D_mifish$post_median,0.50)) %>% 
                  slice(which.min(post_median))
    temp.q.75 <- D_mifish %>% filter(post_median > quantile(D_mifish$post_median,0.75)) %>% 
                  slice(which.min(post_median))
    temp.q.90 <- D_mifish %>% filter(post_median > quantile(D_mifish$post_median,0.90)) %>% 
                  slice(which.min(post_median))
    temp.max <- D_mifish[which.max(D_mifish$post_median),] 
    
    temp.all <- bind_rows(temp.min,temp.q.10,temp.q.25,temp.q.50,temp.q.75,temp.q.90,temp.max)
    temp.all$lab <- c("min","q.10","q.25","q.50","q.75","q.90","max")
    
    plot.hist <- list()
    for(i in 1:nrow(temp.all)){
      pred.val <- rnbinom(1e5,mu=temp.all$post_median[i],size=temp.all$post_tau[i])
      
      plot.hist[[as.name(temp.all$lab[i])]] <- ggplot()+ geom_histogram(aes_string(pred.val),bins=100,alpha=0.5) + 
                                  geom_vline(xintercept=temp.all$post_median[i],color="red") + 
                                  geom_vline(xintercept=temp.all$nReads[i],color="blue") + 
                                  theme_bw() +
...
save(Output,file=here("data",paste0("Stan_Fit_combined_",format(Sys.time(),"%Y%m%d_%H%M"),".RData")))
