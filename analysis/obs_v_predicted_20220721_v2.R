...
po_1a <- Output$D_mifish %>% 
  ...
  theme(
    ...
    axis.title.y = element_text(size = 18, face = "bold"),
    strip.background = element_rect(fill = 'white'),
    axis.line = element_line(colour = 'black')) +  annotate("text", label= paste0("~italic(r)== ", round(cor(Output$D_mifish$nReads, Output$D_mifish$post_mean),2)),
                                                            parse=TRUE,
                                                            x = 1000,
                                                            y = 10,size = 7)
...
ggsave(
  file = here::here("analysis", "figures", "Figure_1_v2.png"),
  width = 12,
  height = 8
)
