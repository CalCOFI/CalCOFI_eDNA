...
bionmial_station_model_df_2_plot %>% 
  ggplot(., aes(x = station, y = ID_main, fill = T_statistic)) + geom_tile()  + scale_fill_viridis_c( option = "plasma",limits=c(-2, 2), breaks=c(-2,-1,-0,1,2)) + geom_text(aes(label =
  Signficant),color="black", size=8) + ylab("Species") + xlab("Station") + theme_bw()+
...
log_sst_model_df2 %>% 
 ...
 # filter(., Type %in% type_to_plot$Type) %>% 
  ...
  scale_colour_manual(values= palette.new$color) + 
  scale_fill_manual(values= palette.new$color) + 
