...
bionmial_station_model_df_2_plot %>% 
  ...
  Signficant),color="white", size=8) + ylab("Species") + xlab("Station") + theme_bw()+
...
log_station_model_df_2_plot_wc %>% 
         mutate(., ID_main=recode(ID_main,`Sebastes`="Sebastes sp.",
                               `Stenobrachius nannochir`="Stenobrachius leucopsarus cold variant")) %>% 
  ggplot(., aes(x = station, y = ID_main, fill = T_statistic)) + geom_tile()  + scale_fill_viridis_c( option = "plasma", limits=c(-3, 3.5), breaks=c(-3,-2,-1,-0,1,2,3)) + geom_text(aes(label =
  Signficant), color="white", size=8) + xlab("Station") + ylab("Species") + theme_bw() +
  theme(axis.text.y=element_text(size=14,
