...
bionmial_station_model_df_2_plot %>% 
 ...
 arrange(`mean(T_statistic)`) -> species_order_s4

bionmial_station_model_df_2_plot$ID_main <- factor(bionmial_station_model_df_2_plot$ID_main, levels=species_order_s4$ID_main)
bionmial_station_model_df_2_plot$station <- factor(bionmial_station_model_df_2_plot$station, levels=c("Pt. Conception" ,     "San Nicholas Island", "San Diego Inshore" ,  "San Diego Offshore"))


    
binomial_trait_color_pallete$ID_main <- factor(binomial_trait_color_pallete$ID_main, levels=species_order_s4$ID_main)

species_order_s4 %>% 
left_join(binomial_trait_color_pallete) -> log_trait_color_pallete_s4
  
bionmial_station_model_df_2_plot %>% 
  ggplot(., aes(x = station, y = ID_main, fill = T_statistic)) + geom_tile()  + scale_fill_distiller(palette = "Spectral", limits=c(-2, 2), breaks=c(-2,-1,-0,1,2)) + geom_text(aes(label =
  Signficant)) + ylab("Species") + xlab("Station") + theme_bw()+
  theme(axis.text.y=element_text(size=14,color=log_trait_color_pallete_s4$species_color),
axis.text.x = element_text(
size = 14,
angle = 30,
hjust = 1
...
) + guides(fill=guide_legend(title="T Statistic"))-> binomial_site_fig
  
  binomial_site_fig
  
  ggsave(
  binomial_site_fig,
  file = here::here("analysis", "figures", "S8_binomial_site_fig.png"),
  width = 12,
  height = 8
  )
...
bionmial_sst_model_df_2_plot %>% 
  group_by(ID_main) %>% 
  dplyr::summarise(mean(T_statistic)) %>% 
 arrange(`mean(T_statistic)`) -> species_order_s5


bionmial_sst_model_df_2_plot$ID_main <- factor(bionmial_sst_model_df_2_plot$ID_main, levels=species_order_s5$ID_main)


binomial_sst_trait_color_pallete$ID_main <- factor(binomial_sst_trait_color_pallete$ID_main, levels=species_order_s5$ID_main)

species_order_s5 %>% 
left_join(binomial_sst_trait_color_pallete) -> log_trait_color_pallete_s5
...
fill = T_statistic
)) + geom_tile()  + scale_fill_distiller(palette = "Spectral", limits=c(-4, 5), breaks=c(-4,-2,-1,-0,1,2,5)) + geom_text(aes(label =
...
legend.title = element_text(size = 15),
axis.line = element_line(colour = 'black')
) + guides(fill=guide_legend(title="T Statistic")) -> binomial_sst_fig
...
log_station_model_df_2_plot %>% 
  ...
  Signficant)) + xlab("Station") + ylab("Species") + scale_fill_distiller(palette = "Spectral", limits=c(-4, 4), breaks=c(-4,-2,-1,-0,1,2,4))+ theme_bw() +
...
y = reorder(ID_main,-T_statistic),
fill = T_statistic
)) + geom_tile()   + geom_text(aes(label =
Signficant)) + xlab("Biomass Association") + ylab("Species") + scale_fill_distiller(palette = "Spectral", limits=c(-5, 7), breaks=c(-5,-4,-2,-1,-0,1,2,4,7))+
theme(
axis.text.x = element_blank(),
panel.background = element_rect(fill = 'white', colour = 'white'),
panel.grid.major = element_blank(),
...
legend.title = element_text(size = 15),
axis.line = element_line(colour = 'black')
)  + guides(fill=guide_legend(title="T Statistic")) -> log_biomass_sst_fig
...
##### S12 Log Biomass Figure Barplot
```{r}
...
log_sst_model_df_plot %>%
          mutate(., ID_main=recode(ID_main,`Sebastes`="Sebastes sp.",
                               `Stenobrachius nannochir`="Stenobrachius leucopsarus cold variant")) %>% 
  ggplot(., aes(y=T_statistic, x = reorder(ID_main, -T_statistic), fill =Type)) +  geom_bar(position="stack", stat="identity")  + scale_fill_manual(values=palette.new$color) + geom_text(aes(label=Signficant, y = T_statistic + 0.2 * sign(T_statistic))) + xlab("Species") + ylab("T Statistic")+  
...
ggsave(log_biomass_sst_fig_v2, file =here::here("analysis","figures","S12_log_biomass_sst_fig_v2.png"), width = 12, height = 8)

```

#Water Column Temp Comparisons
...
##### Binomial | Station Run models
```{r}
# model_holder_wc <- tibble(test_nest_a_wc$ID_main)
# model_holder_wc$model <- list(0)
# 
# for (i in 1:nrow(model_holder_wc)) {
# print(i)
# model_holder_wc$model[[i]] <-
# stan_glmer(
# PA_biomass ~ 0 + (1 + temp_std | station),
# #mod3
# family = "binomial",
# data = test_nest_a_wc$data[[i]],
# prior = normal(0, 4),
# 
# prior_intercept = normal(0, 10),
# #adapt_delta = 0.99,
...
# }
# 
# saveRDS(model_holder_wc, here("data", "model_holder_wc_20210909.RDS"))

model_holder_wc <- readRDS(here("data","model_holder_wc_20210909.RDS"))
```
...
```{r}
focalTaxon = "Triphoturus mexicanus"
idx = which(test_nest_a_wc$ID_main == focalTaxon)

test_nest_a_wc$data[[idx]] %>%
ggplot(aes(y = PA_biomass, x = temp_std)) +
geom_point(color = "black") +
#geom_smooth(method = "glm", method.args = list(family = "binomial"), se = F) +
geom_point(
aes(y = model_holder_wc$model[[idx]]$fitted.values, x = temp_std),
alpha = .5,
color = "red"
) +
geom_smooth(
...
```{r}
model_coeff_holder_wc <- tibble(test_nest_a_wc$ID_main)
model_coeff_holder_wc$model <- list(0)

i = 1
model_coeff_holder_wc <-
as_tibble(as.list(
c(
model_holder_wc$model[[i]]$coefficients %>% as_tibble(rownames = "Coeff")  %>%
pivot_wider(
names_from = Coeff,
names_glue = "{Coeff}_Coeff",
values_from = c(value)
),
model_holder_wc$model[[i]]$ses %>% as_tibble(rownames = "ses")  %>%
pivot_wider(
names_from = ses,
names_glue = "{ses}_ses",
values_from = c(value)
),
posterior_interval(model_holder_wc$model[[i]], regex_pars = "b") %>%  as.data.frame() %>%  as_tibble(rownames = "Coeff") %>%
pivot_wider(
names_from = Coeff,
names_glue = "{Coeff}_{.value}",
values_from = c(`5%`, `95%`)
...
for (i in 2:length(unique(test_nest_a_wc$ID_main))) {
model_coeff_holder_wc %>%
bind_rows(as_tibble(as.list(
c(
model_holder_wc$model[[i]]$coefficients %>% as_tibble(rownames = "Coeff")  %>%
...
model_coeff_holder_wc$ID_main = test_nest_a_wc$ID_main
...
cbind(coeff1_wc, ses1_wc, five1_wc, ninetyfive1_wc) %>%  as_tibble() %>%
mutate(
.,
T_statistic = Coeff_value / ses_value,
Signficant = case_when(
...
) -> bionmial_station_model_df_wc

bionmial_station_model_df_wc %>%
filter(., str_detect(Coeff_name, "temp_std")) %>%
filter(., Signficant != "") -> species_to_keep_fig1_wc
```

##### S9 Binomial Site Fig
```{r}
bionmial_station_model_df_wc %>%
  filter(., ID_main %in% species_to_keep_fig1_wc$ID_main) %>%
  filter(., str_detect(Coeff_name, "temp_std")) -> bionmial_station_model_df_2_plot_wc
  
bionmial_station_model_df_2_plot_wc %>% 
  left_join(trait_edna, by=c("ID_main")) %>% 
  dplyr::select(ID_main,species_color) %>%  distinct()-> binomial_trait_color_pallete_wc


bionmial_station_model_df_2_plot_wc %>% 
  group_by(ID_main) %>% 
  dplyr::summarise(mean(T_statistic)) %>% 
 arrange(`mean(T_statistic)`) -> species_order_s4_wc

bionmial_station_model_df_2_plot_wc$ID_main <- factor(bionmial_station_model_df_2_plot_wc$ID_main, levels=species_order_s4_wc$ID_main)
bionmial_station_model_df_2_plot_wc$station <- factor(bionmial_station_model_df_2_plot_wc$station, levels=c("Pt. Conception" ,     "San Nicholas Island", "San Diego Inshore" ,  "San Diego Offshore"))


    
binomial_trait_color_pallete_wc$ID_main <- factor(binomial_trait_color_pallete_wc$ID_main, levels=species_order_s4_wc$ID_main)

species_order_s4_wc %>% 
left_join(binomial_trait_color_pallete_wc) -> log_trait_color_pallete_s4_wc
  
bionmial_station_model_df_2_plot_wc %>% 
  ggplot(., aes(x = station, y = ID_main, fill = T_statistic)) + geom_tile()   + scale_fill_distiller(palette = "Spectral", limits=c(-2, 2), breaks=c(-2,-1,-0,1,2)) + geom_text(aes(label =
  Signficant)) + ylab("Species") + xlab("Station") + theme_bw()+
  theme(axis.text.y=element_text(size=14,color=log_trait_color_pallete_s4_wc$species_color),
axis.text.x = element_text(
size = 14,
angle = 30,
hjust = 1
...
legend.title = element_text(size = 15),
axis.line = element_line(colour = 'black')
) + guides(fill=guide_legend(title="T Statistic"))-> binomial_site_fig_wc
  
  binomial_site_fig_wc
  
  ggsave(
  binomial_site_fig_wc,
  file = here::here("analysis", "figures", "S9_binomial_site_fig_wc.png"),
  width = 12,
  height = 8
  )
...
### Year - Binomial vs. Water Column Temp

##### Binomial All Run models
```{r}

# 
# model_holder2_wc <- tibble(test_nest_a_wc$ID_main)
# model_holder2_wc$model <- list(0)
# 
# for (i in 1:nrow(model_holder2_wc)) {
# print(i)
# model_holder2_wc$model[[i]] <-
# stan_glm(
# PA_biomass ~ temp_std ,
# #mod3
# family = "binomial",
...
# }
# 
# saveRDS(model_holder2_wc,
# here("data", "model_holder_binomial_wc_only_20210909.RDS"))
model_holder2_wc <-
readRDS(here("data", "model_holder_binomial_wc_only_20210909.RDS"))

good_model <- tibble(model_holder2_wc$`test_nest_a_wc$ID_main`)
good_model$good_model <- list(0)
i = 1
for (i in 1:nrow(model_holder2_wc)) {
good_model$good_model[[i]] <- model_holder2_wc$model[[i]] %>% class()
}

good_model %>%
filter(., good_model != "list") %>%
filter(., good_model != "0") -> models_to_keep_wc
```
...
filter(
.,
`test_nest_a_wc$ID_main` %in% models_to_keep_wc$`model_holder2_wc$\`test_nest_a_wc$ID_main\``
) -> model_holder2_wc

model_coeff_holder_sst_wc <-
as_tibble(as.list(
c(
model_holder2_wc$model[[i]]$coefficients %>% as_tibble(rownames = "Coeff")  %>%
pivot_wider(
names_from = Coeff,
names_glue = "{Coeff}_Coeff",
values_from = c(value)
),
model_holder2_wc$model[[i]]$ses %>% as_tibble(rownames = "ses")  %>%
pivot_wider(
names_from = ses,
names_glue = "{ses}_ses",
values_from = c(value)
),
posterior_interval(model_holder2_wc$model[[i]]) %>%  as.data.frame() %>%  as_tibble(rownames = "Coeff") %>%
pivot_wider(
names_from = Coeff,
names_glue = "{Coeff}_{.value}",
values_from = c(`5%`, `95%`)
...
for (i in 2:length(unique(model_holder2_wc$`test_nest_a_wc$ID_main`))) {
model_coeff_holder_sst_wc %>%
bind_rows(as_tibble(as.list(
c(
model_holder2_wc$model[[i]]$coefficients %>% as_tibble(rownames = "Coeff")  %>%
...
model_coeff_holder_sst_wc$ID_main = model_holder2_wc$`test_nest_a_wc$ID_main`
...
cbind(coeff2_wc, ses2_wc, five2_wc, ninetyfive2_wc) %>%  as_tibble() %>%
mutate(
.,
T_statistic = Coeff_value / ses_value,
Signficant = case_when(
...
) -> bionmial_sst_model_df_wc
```
##### Binomial Figure Water Temp
```{r}
...
bionmial_sst_model_df_wc %>%
  filter(., Signficant != "") %>%
mutate(., Temp = "Temperature") %>%
filter(., str_detect(Coeff_name, "temp_std")) -> bionmial_sst_model_df_2_plot_wc
  
bionmial_sst_model_df_2_plot_wc %>% 
  left_join(trait_edna, by=c("ID_main")) %>% 
  arrange(desc(T_statistic)) %>%  distinct()-> binomial_sst_trait_color_pallete_wc
...
y = reorder(ID_main,-T_statistic),
fill = T_statistic
)) + geom_tile()  + scale_fill_distiller(palette = "Spectral", limits=c(-4, 5), breaks=c(-4,2,-1,-0,1,2,4,5)) + geom_text(aes(label =
Signficant)) + ylab("Species") + xlab("Binomial Association") + theme_bw()+theme(axis.text.y=element_text(size=14,color=binomial_sst_trait_color_pallete_wc$species_color),
axis.text.x = element_blank(),
panel.background = element_rect(fill = 'white', colour = 'white'),
panel.grid.major = element_blank(),
panel.grid.minor = element_blank(),
...
legend.title = element_text(size = 15),
axis.line = element_line(colour = 'black')
) + guides(fill=guide_legend(title="T Statistic"))-> binomial_sst_fig_wc
...
```{r}
# 
# glmer_stan_mod_log_wc <- tibble(test_nest_a_wc$ID_main)
# glmer_stan_mod_log_wc$model <- list(0)
# 
# for (i in 1:nrow(glmer_stan_mod_log_wc)){
#   print(i)
#   glmer_stan_mod_log_wc$model[[i]] <- stan_glmer(log_Normalized.biomass ~ 0 + (1 + temp_std | station),   #mod3
#                              family = "gaussian",
#                              data = test_nest_a_wc$data[[i]],
#                              #prior = normal(0, 4),
#                             #prior_intercept = normal(0, 10),
...
# }
# 
# saveRDS(glmer_stan_mod_log_wc, here("data","glmer_stan_mod_log_wc_20210909.RDS"))

glmer_stan_mod_log_wc <- readRDS(here("data","glmer_stan_mod_log_wc_20210909.RDS"))
...
#geom_smooth(method = "glm", method.args = list(family = "binomial"), se = F) +
geom_point(
aes(y = glmer_stan_mod_log_wc$model[[idx]]$fitted.values, x = temp_std),
alpha = .5,
color = "red"
) +
geom_smooth(
...
```{r}
log_model_coeff_holder_wc <- tibble(test_nest_a_wc$ID_main)
log_model_coeff_holder_wc$model <- list(0)

i = 1
log_model_coeff_holder_wc <-
as_tibble(as.list(
c(
glmer_stan_mod_log_wc$model[[i]]$coefficients %>% as_tibble(rownames = "Coeff")  %>%
pivot_wider(
names_from = Coeff,
names_glue = "{Coeff}_Coeff",
values_from = c(value)
),
glmer_stan_mod_log_wc$model[[i]]$ses %>% as_tibble(rownames = "ses")  %>%
pivot_wider(
names_from = ses,
names_glue = "{ses}_ses",
values_from = c(value)
),
posterior_interval(glmer_stan_mod_log_wc$model[[i]], regex_pars = "b") %>%  as.data.frame() %>%  as_tibble(rownames = "Coeff") %>%
pivot_wider(
names_from = Coeff,
names_glue = "{Coeff}_{.value}",
values_from = c(`5%`, `95%`)
...
log_model_coeff_holder_wc$ID_main = test_nest_a_wc$ID_main
...
cbind(coeff3_wc, ses3_wc, five3_wc, ninetyfive3_wc) %>%  as_tibble() %>%
mutate(
.,
T_statistic = Coeff_value / ses_value,
Signficant = case_when(
...
) -> log_station_model_df_wc
```

##### S11 Log Biomass Site Figure
```{r}
log_station_model_df_wc %>%
  filter(., str_detect(Coeff_name, "temp_std")) %>%
  filter(., Signficant != "") -> species_to_keep_fig3_wc
  
  log_station_model_df_wc %>%
  filter(., ID_main %in% species_to_keep_fig3_wc$ID_main) %>%
  filter(., str_detect(Coeff_name, "temp_std")) ->  log_station_model_df_2_plot_wc
  
log_station_model_df_2_plot_wc %>% 
  left_join(trait_edna, by=c("ID_main")) %>% 
  dplyr::select(ID_main,species_color) %>%  distinct()-> log_trait_color_pallete_wc
  
log_station_model_df_2_plot_wc %>% 
  group_by(ID_main) %>% 
  dplyr::summarise(mean(T_statistic)) %>% 
 arrange(`mean(T_statistic)`) -> species_order_s6_wc

log_station_model_df_2_plot_wc$ID_main <- factor(log_station_model_df_2_plot_wc$ID_main, levels=species_order_s6_wc$ID_main)
log_station_model_df_2_plot_wc$station <- factor(log_station_model_df_2_plot_wc$station, levels=c("Pt. Conception" ,     "San Nicholas Island", "San Diego Inshore" ,  "San Diego Offshore"))


    
log_trait_color_pallete_wc$ID_main <- factor(log_trait_color_pallete_wc$ID_main, levels=species_order_s6_wc$ID_main)

species_order_s6_wc %>% 
left_join(log_trait_color_pallete_wc) -> log_trait_color_pallete_s6_wc
...
log_station_model_df_2_plot_wc %>% 
         mutate(., ID_main=recode(ID_main,`Sebastes`="Sebastes sp.",
                               `Stenobrachius nannochir`="Stenobrachius leucopsarus cold variant")) %>% 
  ggplot(., aes(x = station, y = ID_main, fill = T_statistic)) + geom_tile()  + scale_fill_distiller(palette = "Spectral", limits=c(-3, 3.5), breaks=c(-3,-2,-1,-0,1,2,3)) + geom_text(aes(label =
  Signficant)) + xlab("Station") + ylab("Species") + theme_bw() +
  theme(axis.text.y=element_text(size=14,
                              color=log_trait_color_pallete_s6_wc$species_color),
axis.text.x = element_text(
size = 14,
angle = 30,
hjust = 1
...
legend.title = element_text(size = 15),
axis.line = element_line(colour = 'black')
) + guides(fill=guide_legend(title="T Statistic")) -> log_biomass_sites_fig_wc
  
log_biomass_sites_fig_wc
  
  ggsave(
  log_biomass_sites_fig_wc,
  file = here::here("analysis", "figures", "S11_log_biomass_sites_fig_wc.png"),
  width = 12,
  height = 8
  )
...
# 
# 
# model_holder4_wc <- tibble(test_nest_a_wc$ID_main)
# model_holder4_wc$model <- list(0)
# 
# for (i in 1:nrow(model_holder4_wc)) {
# print(i)
# model_holder4_wc$model[[i]] <-
# stan_glm(
# log_Normalized.biomass ~ temp_std ,
# #mod3
# family = "gaussian",
...
# }
# 
# saveRDS(model_holder4_wc,
# here("data", "model_holder_biomass_wc_only_20210909.RDS"))

model_holder4_wc <-
  readRDS(here("data", "model_holder_biomass_wc_only_20210909.RDS"))
  
  good_model <- tibble(model_holder4_wc$`test_nest_a_wc$ID_main`)
  good_model$good_model <- list(0)
  
  for (i in 1:nrow(model_holder4_wc)) {
  good_model$good_model[[i]] <- model_holder4_wc$model[[i]] %>% class()
  }
...
model_holder4_wc$`test_nest_a_wc$ID_main`
focalTaxon = "Sebastes"
idx = which(test_nest_a_wc$ID_main == focalTaxon)
...
#geom_smooth(method = "glm", method.args = list(family = "binomial"), se = F) +
geom_point(
aes(y = model_holder4_wc$model[[45]]$fitted.values, x = temp_std),
alpha = .5,
color = "red"
) +
geom_smooth(
...
filter(
.,
`test_nest_a_wc$ID_main` %in% models_to_keep$`model_holder4_wc$\`test_nest_a_wc$ID_main\``
) -> model_holder4_wc


model_coeff_holder4_sst_wc <-
as_tibble(as.list(
c(
model_holder4_wc$model[[i]]$coefficients %>% as_tibble(rownames = "Coeff")  %>%
pivot_wider(
names_from = Coeff,
names_glue = "{Coeff}_Coeff",
values_from = c(value)
),
model_holder4_wc$model[[i]]$ses %>% as_tibble(rownames = "ses")  %>%
pivot_wider(
names_from = ses,
names_glue = "{ses}_ses",
values_from = c(value)
),
posterior_interval(model_holder4_wc$model[[i]]) %>%  as.data.frame() %>%  as_tibble(rownames = "Coeff") %>%
pivot_wider(
names_from = Coeff,
names_glue = "{Coeff}_{.value}",
values_from = c(`5%`, `95%`)
...
for (i in 2:length(unique(model_holder4_wc$`test_nest_a_wc$ID_main`))) {
model_coeff_holder4_sst_wc %>%
bind_rows(as_tibble(as.list(
c(
model_holder4_wc$model[[i]]$coefficients %>% as_tibble(rownames = "Coeff")  %>%
...
model_coeff_holder4_sst_wc$ID_main = model_holder4_wc$`test_nest_a_wc$ID_main`
...
cbind(coeff4_wc, ses4_wc, five4_wc, ninetyfive4_wc) %>%  as_tibble() %>%
mutate(
.,
T_statistic = Coeff_value / ses_value,
Signficant = case_when(
...
log_sst_model_df_wc %>%
  filter(., str_detect(Coeff_name, "temp_std")) %>%
  filter(., Signficant != "") -> species_to_keep_fig4_wc

log_sst_model_df_wc %>%
          filter(., ID_main %in% species_to_keep_fig4_wc$ID_main) %>%
mutate(., Temp = "Temperature") %>%
filter(., str_detect(Coeff_name, "temp_std")) -> log_sst_model_df_2_plot_wc
  
log_sst_model_df_2_plot_wc %>% 
  left_join(trait_edna, by=c("ID_main")) %>% 
  arrange(desc(T_statistic)) %>%  distinct()-> logl_sst_trait_color_pallete_wc
...
y = reorder(ID_main,-T_statistic),
fill = T_statistic
)) + geom_tile()  + scale_fill_distiller(palette = "Spectral", limits=c(-5, 7), breaks=c(-5,-2,-1,-0,1,2,4,7)) + geom_text(aes(label =
...
panel.grid.major = element_blank(),
panel.grid.minor = element_blank(),
axis.text.y = element_text(size = 14, angle = 0, colour=logl_sst_trait_color_pallete_wc$species_color),
axis.title.x = element_blank(),
axis.title.y = element_text(size = 18, angle = 90, face = "bold"),
strip.background = element_rect(fill = 'white'),
strip.text.x = element_text(size = 15),
...
legend.title = element_text(size = 15),
axis.line = element_line(colour = 'black')
)  + guides(fill=guide_legend(title="T Statistic"))-> log_biomass_sst_fig_wc

log_biomass_sst_fig_wc
...
log_sst_model_df_wc %>% 
  ...
  filter(., str_detect(Coeff_name,"temp_std")) %>%
  arrange(desc(T_statistic)) %>% 
  left_join(trait_edna, by=c("ID_main")) %>%  dplyr::select(ID_main,Type,species_color)-> color_trait_wc

color_trait_wc %>%  filter(is.na(Type))

log_sst_model_df_wc %>% 
    left_join(trait_edna, by=c("ID_main"))-> log_sst_model_df2_wc

log_sst_model_df2_wc$Type <- as.character(log_sst_model_df2_wc$Type)
log_sst_model_df2_wc$Type <- factor(log_sst_model_df2_wc$Type, levels=c("Benthic North" ,"Benthic Central","Benthic Cosmopolitan","Benthic South", "Coastal Pelagic North", "Coastal Pelagic Central","Mesopelagic North","Mesopelagic Central" , "Mesopelagic Cosmopolitan"  ,"Mesopelagic South"))
...
##### S13 Log Biomass Figure Barplot
```{r}

log_sst_model_df_wc %>% 
    mutate(., Temp = "Temperature") %>% 
  left_join(color_trait_wc) %>% 
  filter(., str_detect(Coeff_name,"temp_std")) -> log_sst_model_df_plot_wc


log_sst_model_df_plot_wc %>%
          mutate(., ID_main=recode(ID_main,`Sebastes`="Sebastes sp.",
                               `Stenobrachius nannochir`="Stenobrachius leucopsarus cold variant")) %>% 
  ggplot(., aes(y=T_statistic, x = reorder(ID_main, -T_statistic), fill =Type)) +  geom_bar(position="stack", stat="identity")  + scale_fill_manual(values=palette.new$color) + geom_text(aes(label=Signficant, y = T_statistic + 0.2 * sign(T_statistic))) + xlab("Species") + ylab("T Statistic") +  
  theme(axis.text.x = element_text(size=6,angle= 30, hjust= 1), 
        panel.background = element_rect(fill='white', colour='white'),
          ...
          legend.text=element_text(size=10),
          legend.title=element_text(size=15),
          axis.line=element_line(colour='black'),legend.position = c(0.8, 0.7)) + labs(fill="Habitat Association")-> log_biomass_sst_fig_v2_wc
log_biomass_sst_fig_v2_wc

ggsave(log_biomass_sst_fig_v2_wc, file =here::here("analysis","figures","S13_log_biomass_wc_fig_v2.png"), width = 12, height = 8)
...
focalTaxon = "Triphoturus mexicanus"
idx = which(test_nest_a$ID_main == focalTaxon)
test_nest_a$data[[idx]]$ID_main <- "Triphoturus mexicanus"
...
idx = which(test_nest_a$ID_main == focalTaxon)
test_nest_a$data[[idx]]$ID_main <- "Stenobrachius lecopsarus - Cold Variant"
...
focalTaxon = "Triphoturus mexicanus"
idx = which(test_nest_a_wc$ID_main == focalTaxon)
test_nest_a_wc$data[[idx]]$ID_main <- "Mexican Lampfish"

model_holder2_wc$`test_nest_a_wc$ID_main`
test_nest_a_wc$data[[idx]] %>%
  data_grid(temp_std = seq_range(temp_std, n = 51)) %>%
  add_fitted_draws(model_holder2_wc$model[[55]]) %>%
  ggplot(aes(x = (temp_std * 1.37 + 14.5), y = PA_biomass)) +
  stat_lineribbon(aes(y = .value), color="darkred") +
  geom_point(data = test_nest_a_wc$data[[idx]], color = "darkred") +
  scale_fill_brewer(palette = "Reds") +
  scale_color_brewer(palette = "Set2") +
  theme(axis.text.x = element_text(size=14), 
        panel.background = element_rect(fill='white', colour='white'),
          ...
          legend.text=element_text(size=10),
          legend.title=element_text(size=15),
          axis.line=element_line(colour='black'))	+ xlab("MWCT ˚C") + ylab("Occurrence") + labs(fill='Confidence Interval') +
  facet_wrap(~ID_main) -> T_mex_binomial_fig_wc
T_mex_binomial_fig_wc
...
focalTaxon = "Stenobrachius nannochir"
idx = which(test_nest_a_wc$ID_main == focalTaxon)
test_nest_a_wc$data[[idx]]$ID_main <- "Stenobrachius lecopsarus - Cold Variant"
model_holder2_wc$`test_nest_a_wc$ID_main`[[48]]

test_nest_a_wc$data[[idx]] %>%
  data_grid(temp_std = seq_range(temp_std, n = 51)) %>%
  add_fitted_draws(model_holder2_wc$model[[48]]) %>%
  ggplot(aes(x = (temp_std * 1.37 + 14.5), y = PA_biomass)) +
  stat_lineribbon(aes(y = .value), color="navy") +
  geom_point(data = test_nest_a_wc$data[[idx]], color = "navy") +
  scale_fill_brewer(palette = "Blues") +
  scale_color_brewer(palette = "Set2") +
  theme(axis.text.x = element_text(size=14), 
        panel.background = element_rect(fill='white', colour='white'),
          ...
          legend.title=element_text(size=15),
          axis.line=element_line(colour='black'))	+ xlab("MWCT ˚C") + ylab("Occurrence") + labs(fill='Confidence Interval') +
  facet_wrap(~ID_main) -> garnet_binomial_fig_wc

garnet_binomial_fig_wc
...
focalTaxon = "Stenobrachius leucopsarus"
idx = which(test_nest_a_wc$ID_main == focalTaxon)
test_nest_a_wc$data[[idx]]$ID_main <- "Stenobrachius lecopsarus - Other Variant"
model_holder2_wc$`test_nest_a_wc$ID_main`[[47]]

test_nest_a_wc$data[[idx]] %>%
  data_grid(temp_std = seq_range(temp_std, n = 51)) %>%
  add_fitted_draws(model_holder2_wc$model[[47]]) %>%
  ggplot(aes(x = (temp_std * 1.37 + 14.5), y = PA_biomass)) +
  stat_lineribbon(aes(y = .value), color="navy") +
  geom_point(data = test_nest_a_wc$data[[idx]], color = "navy") +
  scale_fill_brewer(palette = "Blues") +
  ...
  theme(axis.text.x = element_text(size=14), 
        panel.background = element_rect(fill='white', colour='white'),
          ...
          legend.title=element_text(size=15),
          axis.line=element_line(colour='black'))	+ xlab("MWCT ˚C") + ylab("Occurrence") + labs(fill='Confidence Interval') +
  facet_wrap(~ID_main) -> garnet_binomial_fig_wc_other

garnet_binomial_fig_wc_other
...
biomass_conc <- ggplot(data=b_long_all_edit_site_conception, aes(x=Year)) + 
    geom_rect_pattern(
    aes(
      xmin=2014, ymin=-3, xmax=2019, ymax=18,
      pattern_angle   = I(45),
...
biomass_sni <- ggplot(data=b_long_all_edit_site_sni, aes(x=Year)) + 
  ...
  geom_line(aes(y=`median_log`,group=Species, color=Species),size=1, na.rm = TRUE) +
  geom_errorbar(aes(ymin = p05_log, ymax = p95_log,group=Species, color=Species), width = 0.1)  +
geom_line(data=sni_temp, aes(x=Year, y=(coeff2+two_month_sst*coeff1)), size = 2, color= dar4[2]) +
scale_y_continuous(
...
biomass_sdi <- ggplot(data=b_long_all_edit_site_sdi, aes(x=Year)) + 
  ...
  geom_line(aes(y=`median_log`,group=Species, color=Species),size=1, na.rm = TRUE) +
  geom_errorbar(aes(ymin = p05_log, ymax = p95_log,group=Species, color=Species), width = 0.1)  +
geom_line(data=sdi_temp, aes(x=Year, y=(coeff2+two_month_sst*coeff1)), size = 2, color= dar4[3]) +
scale_y_continuous(
...
biomass_sdo <- ggplot(data=b_long_all_edit_site_sdo, aes(x=Year)) + 
  ...
  geom_line(aes(y=`median_log`,group=Species, color=Species),size=1, na.rm = TRUE) +
  geom_errorbar(aes(ymin = p05_log, ymax = p95_log,group=Species, color=Species), width = 0.1)  +
geom_line(data=sdo_temp, aes(x=Year, y=(coeff2+two_month_sst*coeff1)), size = 2, color= dar4[4]) +
scale_y_continuous(
...
biomass_conc_wc <- ggplot(data=b_long_all_edit_site_conception, aes(x=Year)) + 
  ...
  geom_line(aes(y=`median_log`,group=Species, color=Species),size=1, na.rm = TRUE) +
  geom_errorbar(aes(ymin = p05_log, ymax = p95_log,group=Species, color=Species), width = 0.1)  +
geom_line(data=conc_temp, aes(x=Year, y=(coeff2+mean_100_T*coeff1)), size = 2, color= dar4[1]) +
scale_y_continuous(
...
MHW_species_summary_sig_all %>% 
          mutate(., ID_main=recode(ID_main,`Sebastes`="Sebastes sp.",
                               `Stenobrachius nannochir`="Stenobrachius leucopsarus cold variant"))%>% 
  ggplot(., aes(y= mean, x= reorder(ID_main, -mean), fill=Type)) +  geom_bar(position="stack", stat="identity") +
  scale_fill_manual( values = palette.new8$color) + ylab(expression(Delta*"Abundance")) + guides(fill=guide_legend(title="Habitat Association"))  +geom_text(aes(label=Sig_MHW, y = mean + 0.2 * sign(mean)))+
  theme(axis.text.x = element_text(size=12,angle= 45, hjust= 1), 
        panel.background = element_rect(fill='white', colour='white'),
          panel.grid.major = element_blank(), 
          panel.grid.minor = element_blank(), 
          ...
          axis.line=element_line(colour='black') )  + facet_grid(station~ .)   -> mhw_differential_abundance_all
...
# Figure Sum Habitat vs. WC

```{r}
b_ID_main_out_type %>% 
  left_join(mean_100_T_mean) %>%
  filter(., Type =="Mesopelagic South") %>% 
  ggplot(., aes(y=log_sum, x=mean_mean_100_T, colour=Type, fill = Type, group=Type)) +geom_point() +geom_smooth(method="lm") +facet_wrap(.~Type) + scale_fill_manual(values="#B40F20") + scale_colour_manual(values="#B40F20") +
  stat_poly_eq(formula = y~x, 
                aes(label = paste(..rr.label..,..p.value.label.., sep = "~~~")), 
                parse = TRUE,
               label.x = 1.5,
  ...
  theme( panel.background = element_rect(fill='white', colour='white'),
          ...
          legend.title=element_text(size=15),
         axis.line=element_line(colour='black'),
         legend.position = "none") + xlab("MWCT ˚C") + ylab("Log(Abundance)")  -> sum_habitat_temp_regression_wc
sum_habitat_temp_regression_wc
...
b_ID_main_out_type %>% 
  left_join(mean_100_T_mean) %>% 
  ggplot(., aes(y=log_sum, x=mean_mean_100_T, colour=Type, fill = Type, group=Type)) +geom_point() +geom_smooth(method="lm") +facet_wrap(.~Type, scales="free") + scale_fill_manual(values=palette.new$color) + scale_colour_manual(values=palette.new$color) +
  stat_poly_eq(formula = y~x, 
                aes(label = paste(..eq.label.., ..rr.label..,..p.value.label.., sep = "~~~")), 
                parse = TRUE,
               label.x = 0.5,
  ...
  theme( panel.background = element_rect(fill='white', colour='white'),
          ...
          strip.text.x=element_text(size=14, face='italic'),
          legend.text=element_text(size=10),
          legend.title=element_text(size=15)) + xlab("Average Water Column Temp ˚C") + ylab("Mean of Log Abundance Estimates")  -> mean_habitat_temp_regression_wc

mean_habitat_temp_regression_wc
...
b_long_meso_S_edit %>% 
  left_join(mean_100_T) %>% 
  ggplot(., aes(y=mean_log, x=mean_100_T, colour=ID_main, fill = ID_main, group=ID_main)) +geom_point() +geom_smooth(method="lm")+facet_wrap(.~ID_main)   + scale_fill_manual(values = c("orangered2","tomato","red3","red4")) + scale_colour_manual(values = c("orangered2","tomato","red3","red4")) +
  stat_poly_eq(formula = y~x, 
                aes(label = paste(..eq.label.., ..rr.label..,..p.value.label.., sep = "~~~")), 
                parse = TRUE,
               label.x = 0.5,
  ...
  theme( panel.background = element_rect(fill='white', colour='white'),
          ...
          legend.text=element_text(size=10),
          legend.title=element_text(size=15),
         legend.position = "none") + xlab("Average Water Column Temp ˚C") + ylab("Log(Abundance)") +ylim(-4,2) -> habitat_temp_regression_meso_pelagic_wc

habitat_temp_regression_meso_pelagic_wc
...
binomial_sst_fig +labs(tag = "A") -> binomial_sst_fig_tagged
garnet_binomial_fig + labs(tag = "B") -> garnet_binomial_fig_tagged
T_mex_binomial_fig + labs(tag = "C") -> T_mex_binomial_fig_tagged


leggest + binomial_sst_fig + garnet_binomial_fig + T_mex_binomial_fig + plot_annotation(tag_levels = "A")

(leggest + binomial_sst_fig_tagged  + plot_layout(byrow = F, widths = c(1,3), ncol = 2)) -> a1

 b1 <-  (garnet_binomial_fig_tagged + T_mex_binomial_fig_tagged + plot_layout(byrow = F, widths = c(3,3), ncol = 2))
...
### Figure 2 SST Biomass
```{r}

log_biomass_sst_fig +labs(tag = "A") -> log_biomass_sites_fig_tagged
...
binomial_sst_fig_wc +labs(tag = "A") -> binomial_sst_fig_tagged_wc
garnet_binomial_fig_wc + labs(tag = "B") -> garnet_binomial_fig_tagged_wc
T_mex_binomial_fig_wc + labs(tag = "C") -> T_mex_binomial_fig_tagged_wc


(leggest + binomial_sst_fig_tagged_wc  + plot_layout(byrow = F, widths = c(1,3), ncol = 2)) -> a1_wc

 b1_wc <-  (garnet_binomial_fig_tagged_wc + T_mex_binomial_fig_tagged_wc + plot_layout(byrow = F, widths = c(3,3), ncol = 2))
 
 a1_wc/b1_wc

ggsave(filename = here("analysis","figures","Figure_S6_combo_wc.png"),
       width=18,
       height = 12,
       dpi = 400,
       units = c("in"))
