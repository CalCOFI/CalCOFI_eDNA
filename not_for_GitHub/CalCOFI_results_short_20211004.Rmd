...
saveRDS(b_master_out, here("data","b_master_out.RDS"))

b_master_out <- readRDS(file = here("data", "b_master_out.RDS"))
...
saveRDS(b_master_out_type, here("data","b_master_out_type.RDS"))

b_master_out_type <- readRDS(file = here("data", "b_master_out_type.RDS"))
...
saveRDS(b_master_out_site, here("data","b_master_out_site.RDS"))

b_master_out_site <- readRDS(file = here("data", "b_master_out_site.RDS"))
...
saveRDS(b_master_dist_out, here("data","b_master_dist_out.RDS"))

b_master_dist_out <- readRDS(file = here("data", "b_master_dist_out.RDS"))
...
b_grid_tibble_2 %>%
  ...
  b_master_dist_out_station %>% 
    pivot_wider(values_from = mean_b_est, names_from =
  MHW) %>%
  mutate(., `MHW-Before` = MHW - Before) -> b_master_dist_out_station
  
  saveRDS(b_master_dist_out_station, here("data","b_master_dist_out_station.RDS"))

b_master_dist_out_station <- readRDS(file = here("data", "b_master_dist_out_station.RDS"))
```
...
model_holder4$`test_nest_a$ID_master`
focalTaxon = "Sebastes"
idx = which(test_nest_a$ID_master == focalTaxon)
...
coeff2 <- -5
  ...
  biomass_conc <- ggplot(data=b_long_all_edit_site_conception, aes(x=Year)) + 
    geom_rect_pattern(
    aes(
      xmin=2014, ymin=-3, xmax=2019, ymax=15,
      pattern_angle   = I(45),
      pattern_colour  = I("grey80"),
  ...
  geom_line(aes(y=`median_log`,group=Species, color=Species),size=0.5, na.rm = TRUE) +
  geom_errorbar(aes(ymin = p05_log, ymax = p95_log,group=Species, color=Species), width = 0.1)  +
geom_line(data=conc_temp, aes(x=Year, y=(two_month_sst/coeff1)), size = 1, color= dar4[1]) +
scale_y_continuous(
    ...
    sec.axis = sec_axis(~.*coeff1, name="SST ˚C")
  ) +
  theme_bw() +		## makes background white
  theme(  panel.background = element_rect(fill='white', colour='white'),
          panel.grid.major = element_blank(), 
...
mutate(., log_est = log(est + 0.1))  -> b_master_dist_out_2

b_master_dist_out_2 %>% 
  filter(., ID_master %in% c("Engraulis mordax","Triphoturus mexicanus","Stenobrachius nannochir","Sebastes","Merluccius productus")) %>%
  ggplot(., aes(y= log_est, x= MHW, color=ID_master)) +  geom_boxplot() + facet_grid(ID_master~., scales = "free")

```


pairwise matrix
each vsue is correlation of biomass
A is neg with B (could be replacing, any number of reasons)
climate change, eclogical 
...
b_grid_tibble_2 %>%
  ...
  mutate(., log_est = log(est + 0.01)) -> b_master_dist_out_station_v2

b_master_dist_out_station_v2 %>% 
  filter(., ID_master %in% c("Engraulis mordax","Sardinops sagax","Triphoturus mexicanus","Stenobrachius nannochir","Sebastes","Merluccius productus")) %>% 
  ggplot(., aes(y= log_est, x= MHW, color=ID_master)) +  geom_boxplot() + facet_grid(ID_master~station, scales = "free")
...
b_master_out_site %>% 
   dplyr::select(Year, station, ID_master, median) %>% 
filter(., median > 0) %>% 
  group_by(ID_master) %>% count() %>% 
...
b_master_out_site %>% 
  ...
  filter(., ID_master %in% species_to_choose$ID_master) %>% 
  arrange(ID_master) %>% 
  pivot_wider(names_from=ID_master, values_from=median) %>%  as.data.frame()-> eDNA_all_wide
...
trait_edna_1 %>% 
  filter(., ID_master %in% species_to_choose$ID_master) %>% 
  arrange(ID_master) %>% select(ID_master, Type, Habitat    ,    Range) ->trait_edna_gllvm
trait_edna_gllvm$ID_master -> rownames(trait_edna_gllvm)
trait_edna_gllvm %>% dplyr::select(-ID_master) -> trait_edna_gllvm
...
rownames(cr) %>% as_tibble(.) %>% 
  dplyr::select(ID_master=value) %>% 
  left_join(trait_edna) -> cr_traits
LabelCol = cr_traits$species_color

corrplot(cr,order="original",diag = TRUE, type = "lower", 
         method = "square", tl.cex = 0.5, tl.srt = 45, tl.col = LabelCol, p.mat = testRes$p, sig.level = c(0.001, 0.01, 0.05), pch.cex = 0.9, 
...
ordiplot(fit_env, biplot = TRUE, ind.spp = 25, jitter = TRUE, cex.spp = 1,
...
```{r}
ordiplot(fit4lv, biplot = TRUE, ind.spp = 25, jitter = TRUE, cex.spp = 1,
...
fit_4th <- gllvm(eDNA_all_wide_2, env_calcofi, trait_edna_gllvm, family = "negative.binomial", num.lv = 3, 
                 formula = y ~ (two_month_sst) +
                (two_month_sst) : (Type + 
                Habitat + Range), seed = 123,
                row.eff = "random", control.start =list(n.init = 3, jitter.var = 0.01),
                randomX = ~ two_month_sst)
...
fit_4th2 <- gllvm(eDNA_all_wide_2, env_calcofi, trait_edna_gllvm, family = "negative.binomial", num.lv = 3, 
                 formula = y ~ (two_month_sst) , seed = 123,
                row.eff = "random", control.start =list(n.init = 3, jitter.var = 0.01),
                randomX = ~ two_month_sst)
